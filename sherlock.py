...
def api_data():
    ...
    for chat_id, chat_msgs in chats_map.items():
        # Get all unique senders in this chat (preserve order: first sender, then receiver)
        senders_ordered = []
        seen_ids = set()
        
        for m in chat_msgs:
            if m["user_id"] and m["user_id"] not in seen_ids:
                senders_ordered.append({
                    "id": m["user_id"],
                    "name": m["sender_name"],
                    "username": m["username"]
                })
        ...
        # Build contact name showing both sides (sender | receiver)
        if len(senders_ordered) >= 2:
            contact_name = f"{senders_ordered[0]['name']} | {senders_ordered[1]['name']}"
        elif len(senders_ordered) == 1:
            contact_name = senders_ordered[0]['name']
        else:
            contact_name = f"Chat {chat_id}"
        
        # Use first sender's username for reference
        contact_username = senders_ordered[0]["username"] if senders_ordered and senders_ordered[0]["username"] else None
...
# Global dict to track business_connection_id -> owner_user_id
business_connections = {}


async def track_business_connection(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """Track all updates to extract business connection info"""
    # Try to extract business connection from any update type
    if hasattr(update, 'business_connection') and update.business_connection:
        conn = update.business_connection
        business_connections[conn.id] = conn.user_id
        log.info("Business connection tracked: %s -> user %s", conn.id, conn.user_id)
    
    # Also track from business messages
    if hasattr(update, 'business_message') and update.business_message:
        msg = update.business_message
        if msg.business_connection_id and msg.business_connection_id not in business_connections:
            # Try to infer owner from message context
            if hasattr(update, 'business_connection') and update.business_connection:
                business_connections[msg.business_connection_id] = update.business_connection.user_id
...
async def on_business_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get business owner ID from connection tracking
    business_owner_id = business_connections.get(msg.business_connection_id, ADMIN_ID)
    
    # If not in cache yet, try to extract from update
    if business_owner_id == ADMIN_ID and hasattr(update, 'business_connection') and update.business_connection:
        business_owner_id = update.business_connection.user_id
        business_connections[msg.business_connection_id] = business_owner_id
...
async def on_edited_business_message(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Get original message sender from cache
    cached = fetch_cached(msg.message_id, msg.chat.id)
    original_sender_id = cached["user_id"] if cached else None
    
    # Skip if the person who sent the message is editing their own message
    if user_id and user_id == original_sender_id:
        log.info("Skipping sender's own edit: msg_id=%s, user=%s", msg.message_id, user_id)
        # Still cache the edit
...
async def on_deleted_business_messages(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for mid in message_ids:
        ...
        # Skip if the original sender deleted their own message (don't notify anyone)
        if cached and original_sender_id:
            log.info("Skipping sender's own deleted msg id=%s, sender=%s", mid, original_sender_id)
...
async def async_main() -> None:
    ...
    app.add_handler(TypeHandler(Update, track_business_connection), group=-1)  # Run first
