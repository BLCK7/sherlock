...
async def on_business_connection(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    """Track business connections to know which user owns which business account"""
    conn = update.business_connection
    if not conn:
        return
    
    business_connections[conn.id] = conn.user_id
    log.info("Business connection: %s -> user %s", conn.id, conn.user_id)
...
async def async_main() -> None:
    ...
    app.add_handler(MessageHandler(filters.UpdateType.BUSINESS_CONNECTION, on_business_connection))
